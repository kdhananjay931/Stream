...
class Employee {
    ...
    public Employee(int id, String name, int age, String gender, String department,
                    int dateOfJoining, double salary) {
        this.id = id;
        this.name = name;
        this.age = age;
        this.gender = gender;
...
//   Map<String, Long> genderCounting = list.stream().collect(Collectors.groupingBy(Employee::getGender, Collectors.counting()));
//
//        System.out.println(genderCounting);

        Optional<Employee> sal = list.stream().max(Comparator.comparing(Employee::getSalary));

        Employee highest=sal.get();
